...
dotenv.config();
export function getActiveRooms(io) {
    try {
        ...
        */
        ...
        for (const room of filteredActiveRooms) {
            if (room[0] !== 'lobby' && !room[0].startsWith('DM'))
                res.push(room[0]); //* add room to res if it's not lobby or a DM room
        }
        return res;
    }
    catch (error) {
 ...
 */
export async function chatRoomExists(roomName) {
 ...
 */
export async function directMessageRoomExists(directMessageRoomName) {
 ...
 */
export async function insertChatRoom(roomName) {
    try {
...
*/
export async function insertDirectMessageRoom(directMessageRoomName, user1_id, user2_id) {
    try {
 ...
 */
export async function handleDMRoomJoin(io, socket, room) {
 ...
 * @returns Promise<void>
 */
export async function handleChatRoomJoin(io, socket, room, roomIDs, allRooms) {
    ...
    if (!roomIDs.has(room))
        roomIDs.set(room, chatroom_id); //! -> roomIDs = { 'lobby' => postgres chatroom_id }
    if (!allRooms.includes(room))
        allRooms.push(room); //! add room to allRooms array, if it doesn't exist
    //* join new room
    socket.room = room;
    socket.join(socket.room);
    ...
    if (!room.startsWith('DM')) {
        io.to(room)
            .emit("systemMessage", { message: `${socket.username} has joined the chat` });
    }
 ...
 * @returns Promise<void>
 */
export async function handleStartDM(username, io, socket) {
    ...
    let targetSocket = Array.from(io.sockets.sockets.values()).find((s) => s.username === username);
    if (!targetSocket) {
    ...
    const user2_id = targetSocket === null || targetSocket === void 0 ? void 0 : targetSocket.userID;
    const user1_id = socket === null || socket === void 0 ? void 0 : socket.userID;
    ...
    if (userHasDMRoom(socket, directmessageroom_name) || userHasDMRoom(targetSocket, directmessageroom_name)) {
        try {
            ...
            if (exists)
                throw new Error('DM room already exists');
