...
router.get('/getallusers', getAllUsers, (_req, res) => {
...
// });
router.post('/registeruser', registerUser, setUserIDRedis, (_req, res) => {
    res.status(200).json('user registered');
});
router.post('/userlogin', userLogIn, createJWT, (_req, res) => {
    const username = res.locals.user.username;
    res.status(200).json({ username });
});
router.delete('/deleteuser', verifyJWT, deleteUser, deleteJWT, (req, res) => {
    res.status(200).json('user deleted');
});
router.patch('/updateuser', verifyJWT, updateUser, createJWT, (req, res) => {
    res.status(200).json({
...
router.delete('/userlogout', deleteJWT, (_req, res) => { res.status(200).json('user logged out'); });
// Chatroom routes
// intend to add verifyJWT before and createJWT(renew JWT) after addchatroom because it is a user operation
router.get('/getallchatrooms', verifyJWT, getAllChatrooms, createJWT, (req, res) => {
    res.status(200).json(res.locals.allChatrooms);
});
router.post('/addchatroom', verifyJWT, addChatroom, setChatroomIDRedis, createJWT, (req, res) => {
...
router.post('/addchatlog', verifyJWT, addChatlog, addChatroomLogRedis, createJWT, (req, res) => {
    res.status(200).json(res.locals.chatlog);
});
// Getting userID and chatroomID routes
router.post('/getChatHistory', getChatroomID, getChatHistoryRedis, (req, res) => {
...
// Route for images
router.post('/getSignedURL', getAWSURL, (req, res) => {
...
router.get('/verify', verifyJWT, (_req, res) => {
